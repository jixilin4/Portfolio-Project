...
For the kernel implementation of Itinerary, I selected Sequence<Leg> as the underlying representation.

I made this choice because the itinerary is restricted to at most two legs.
Using a sequence makes the kernel operations, such as add to the front,
add to the end, remove from the front, and remove from the end straightforward
and computationally efficient. Since the number of legs is small and bounded,
indexed access will not introduce unnecessary complexity.
